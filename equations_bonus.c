...
/*   Updated: 2023/02/06 19:02:42 by joalmeid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mandelbrot(double x, double y)
{
	...
	z_r = x;
	z_im = y;
	while (my_pow(z_r, 2) + my_pow(z_im, 2) <= 4 && i < limit)
	{
		new_z_r = my_pow(z_r, 2) - my_pow(z_im, 2) + x;
		z_im = 2 * z_r * z_im + y;
		z_r = new_z_r;
		i++;
	}
...
int	julia(double x, double y, double c_r, double c_im)
{
	...
	while (my_pow(z_r, 2) + my_pow(z_im, 2) <= 4 && i < limit)
	{
		new_z_r = my_pow(z_r, 2) - my_pow(z_im, 2) + c_r;
